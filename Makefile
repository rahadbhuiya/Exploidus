...
KERNEL_C_SRCS := \
    ...
    kernel/drivers/keyboard.c kernel/drivers/fb.c kernel/drivers/font.c kernel/drivers/mouse.c kernel/drivers/fb_console.c kernel/drivers/ata.c kernel/drivers/driver.c kernel/drivers/rtc.c kernel/drivers/blockdev.c \
...
SSHD_C_SRCS    := userspace/bin/sshd.c userspace/bin/sshd_packet.c
...
SSHD_OBJS := $(patsubst %.c,  build/%.o, $(SSHD_C_SRCS))
...
build/userspace/bin/sshd.elf: $(BIN_OBJS) $(SSHD_OBJS) userspace/bin/pie.ld
	@mkdir -p $(dir $@)
	@echo "[LD]  sshd   -> $@"
	$(LD) -T userspace/bin/pie.ld $(PIE_LDFLAGS) -o $@ $(BIN_OBJS) $(SSHD_OBJS)
	x86_64-elf-strip --strip-debug $@
